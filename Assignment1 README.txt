...
-the local version of this Assignment runs best in Mozilla Firefox
-http://magnusandy.github.io/350 is the hosted version of this site on github that works on mobile (using a html5 compatable browser like chrome)
...
Task B:
	- The "You started from here!" marker IS created, but it gets covered by the current
	  ...
	  so check the title of the marker before the second popup to see it.
Task D:
...
Bonus: Attempted to use the Application Cache, everything but the map seems to cache, not sure why this is. It might not be possible to 
	   directly cache a map. After some research, it seems as though, caching a map is not possible, through the use of application cache.
